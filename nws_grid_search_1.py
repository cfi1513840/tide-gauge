...
def search(args: argparse.Namespace) -> list[Match]:
    ...
    print(
        ...
        f"searching for {', '.join(args.fields)}.",
        file=sys.stderr,
    )
    ...
    for radius in range(args.max_rings + 1):
        ...
        print(f"Completed ring {radius}; matches so far: {len(matches)}", file=sys.stderr)
...
def parse_args() -> argparse.Namespace:
    ...
    if args.max_rings < 0 or args.limit < 1 or args.delay < 0 or args.timeout <= 0:
        parser.error("max-rings/delay cannot be negative; limit/timeout must be positive")
    return args
